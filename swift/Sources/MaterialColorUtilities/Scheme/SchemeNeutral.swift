...
public class SchemeNeutral: DynamicScheme {
  public convenience init(sourceColorHct: Hct, isDark: Bool, contrastLevel: Double) {
    let palettes = CorePalettesNeutral(sourceColorHct: sourceColorHct)
    self.init(
...
/// Use [SchemeNeutralProvider] when you need to create multiple [SchemeNeutral] from the same
/// source color.
///
/// This provider reduces overlapped computation by reusing tonal palettes.
public struct SchemeNeutralProvider: DynamicSchemeProvider {
  ...
  public init(sourceColorHct: Hct) {
    ...
    self.palettes = CorePalettesNeutral(sourceColorHct: sourceColorHct)
  }
...
/// Core palettes of [SchemeNeutral].
private struct CorePalettesNeutral: CorePalettes {
  ...
  init(sourceColorHct: Hct) {
    self.primary = TonalPalette.of(sourceColorHct.hue, 12.0)
    self.secondary = TonalPalette.of(sourceColorHct.hue, 8.0)
    self.tertiary = TonalPalette.of(sourceColorHct.hue, 16.0)
    self.neutral = TonalPalette.of(sourceColorHct.hue, 2.0)
    self.neutralVariant = TonalPalette.of(sourceColorHct.hue, 2.0)
  }
}
